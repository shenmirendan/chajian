...
// v 1.6 加入白名单的图片将从本地鉴定记录删除其数据以节省资源。  新增收藏图片和查看已收藏图片功能。
...
// 	let url = "https://c2cpicdw.qpic.cn/offpic_new/0//1761869682-4110036598-999999999999666777776991A66677777766605/0"
// 	let res = await localQuery(url)
// 	console.log(res)
...
export async function report(e) {
	...
	msg = msg.concat([`\n\n合计 `, segment.text((total * (5 / 10000)).toFixed(2)), " 元"])
	e.reply(msg)
	return true
}
...
// ================将收藏夹的图片以合并转发消息的形式发送===============
export async function showcollect(e) {

	// 如果要允许所有人查看收藏，把下面这一行注释掉
	if (!e.isMaster) return false

	// 如果要允许在群内查看收藏，把下面四行注释掉
	if (e.isGroup) {
	  e.reply("仅可私聊查看")
	  return true
	}
	...
	let data = await readJSON(dirpath, filename)
	if (!data || data.temp1.length == 0) {
		e.reply("还没有收藏哦")
		return true
	}
	var data_msg = []
	for (let item of data.temp1) {
		data_msg.push({
	...
	if (e.isGroup)
		e.reply(await e.group.makeForwardMsg(data_msg.reverse()))
	else
		e.reply(await e.friend.makeForwardMsg(data_msg.reverse()))
	return true; //返回true 阻挡消息不再往下
}


// ====================收藏图片===================
export async function collect(e) {
	...
	for (let val of e.message) {
		switch (val.type) {
			case "flash":
			case "image":
				if (await checkcollect(val.url)) {
					existed++
				}
				else {
					// console.log(val.url)
					await addcollect(val.url)
					count++
				}
		}
	}
	if (count || existed)
		e.reply([
			count ? `成功收藏${count}张图片~` : "未能收藏指定图片,", existed ? `\n有${existed}张图片已在收藏夹中~` : ""
		])
	else
...
// ====================取消收藏图片===================
export async function uncollect(e) {
	if (!e.isMaster) return true
	if (e.hasReply) {
		// console.log(e);
		let reply;
	...
	if (total)
		e.reply([
			count ? `成功取消收藏${count}张图片~` : "未能取消收藏指定图片,", total - count ? `\n有${total - count}张图片不在收藏夹中~` : ""
		])
	else
		e.reply("请在命令后带上图片,或对图片回复此命令")
	return true
...
// ==============将一张图加入到收藏夹,参数是这张图的url=============
export async function addcollect(url) {
	let md5 = await getmd5(url)
	// console.log(md5)
	// 构造一个数组成员
	let newCollect = {
...
// =================判断收藏夹中有没有这个图，参数是这张图的url==============
export async function checkcollect(url) {
	let md5 = await getmd5(url)
	// 读出json
	let data = await readJSON(dirpath, filename)
	// 逐个判断bmd中的每个成员
	for (var member of data.temp1) {
		// 如果该成员的MD5值等于md5,就返回true
		if (member.MD5 == md5)
			return true
	}
...
// =================从收藏夹删除一张图，参数是这张图的url===============
export async function delcollect(url) {
	let md5 = await getmd5(url)
	// 读出json
	let data = await readJSON(dirpath, filename)
	for (var i = 0; i < data.temp1.length; i++) {
		// 如果该MD5值等于md5,就去掉这个数组成员
		if (data.temp1[i].MD5 == md5) {
			data.temp1.splice(i, 1)
			// 把处理后的data写回json文件里
			await writeJson(dirpath, filename, data)
			return true
		}
...
// =================从本地鉴定记录删除一张图的记录，参数是这张图的url===============
export async function deljdjg(url) {
	let md5 = await getmd5(url)
	// 读出json
	let data = await readJSON(dirpath, filename)
	for (var i = 0; i < data.jdjg.length; i++) {
		// 如果该MD5值等于md5,就去掉这个数组成员
		if (data.jdjg[i].MD5 == md5) {
			data.jdjg.splice(i, 1)
			// 把处理后的data写回json文件里
			await writeJson(dirpath, filename, data)
			return true
		}
